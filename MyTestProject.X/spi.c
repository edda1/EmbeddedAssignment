...
void spi_setup(void) {
    ...
    TRISDbits.TRISD6 = 0;       // RD6 ? CS3: BMM150 magnetometer
    TRISBbits.TRISB4 = 0;       // RB4 ? CS2: Gyroscope
    TRISBbits.TRISB3 = 0;       // RB3 ? CS1: BMI088 accelerometer
...
void accel_bw(int bw) {
    ...
    switch (bw) {
        ...
        default:
            // If a wrong value is received, send it to UART
            LATBbits.LATB3 = 1;
            uart_transmit("$ERR,1*");
            return;
    }
...
Sensor_DataStruct accel_read() {
    ...
    spi_write(0x02 | 0x80);         // Address byte, read flag set 
    ...
    //Convert raw counts to g: sensitivity = 0.00098 g/LSB (for ±2g range)
    //then derive tilt angles using atan2.
